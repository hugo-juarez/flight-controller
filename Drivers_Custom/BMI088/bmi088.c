...
static FC_Status_t BMI088_Accel_ReadRegister(const BMI088_t *bmi088, const BMI088_AccelReg_t reg, uint8_t *data)
{
    ...
    uint8_t rx_buffer[3];
    if ( HAL_SPI_TransmitReceive(bmi088->bmi088_config.spi, tx_buffer, rx_buffer, 3, HAL_MAX_DELAY) != HAL_OK) return FC_SPI_ERR;
...
static FC_Status_t BMI088_Accel_WriteRegister(const BMI088_t *bmi088, const BMI088_AccelReg_t reg, const uint8_t data)
{
    ...
    if ( HAL_SPI_Transmit(bmi088->bmi088_config.spi, tx_buffer, 2, HAL_MAX_DELAY) != HAL_OK) return FC_SPI_ERR;
